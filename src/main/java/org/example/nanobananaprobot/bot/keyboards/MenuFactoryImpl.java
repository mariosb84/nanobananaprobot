...
public class MenuFactoryImpl implements MenuFactory {
    ...
    public SendMessage createPromptsExamplesMenu(Long chatId) {
        ...
        String text = "🚀 *Примеры промптов для генераций*\n\n";
        text += "📝 *Скопируйте промпт и используйте в боте:*\n\n";

        text += "🎨 *Фотореалистичный портрет:*\n";
        text += "```\n";
        text += "СТРОГО сохранить внешность 1:1. Не изменять черты лица, возраст, пропорции.\n";
        text += "Локация/фон: аэродром с вертолётом на закате;\n";
        text += "освещение: тёплый боковой свет слева, мягкие тени;\n";
        text += "ракурс/кадр: средний план тела, лёгкий нижний ракурс;\n";
        text += "поза: человек в чёрном спортивном костюме сидит на пороге открытой дверцы вертолета;\n";
        text += "стиль: фотореализм, кино; глубина резкости: средняя;\n";
        text += "фокусное расстояние: 50 мм; атмосферная спокойная энергия кадра.\n";
        text += "```\n\n";

        text += "🏞️ *Пейзаж с атмосферой:*\n";
        text += "```\n";
        text += "Локация: горное озеро на рассвете, отражение гор в воде;\n";
        text += "освещение: мягкий рассеянный свет, туман над водой;\n";
        text += "стиль: фотореализм, национальный парк;\n";
        text += "элементы: сосны по берегам, скалы, кристально чистая вода;\n";
        text += "настроение: спокойствие, уединение, природная гармония.\n";
        text += "```\n\n";

        text += "🎭 *Креативный арт-портрет:*\n";
        text += "```\n";
        text += "СТРОГО сохранить лицо 1:1;\n";
        text += "стиль: киберпанк, неон-нуар;\n";
        text += "элементы: голографические элементы на лице, неоновая подсветка;\n";
        text += "фон: дождливый мегаполис ночью, отражения в лужах;\n";
        text += "освещение: контровой неоновый свет, цветные тени;\n";
        text += "настроение: загадочность, технологичность.\n";
        text += "```\n\n";

        text += "🍕 *Предметная съёмка:*\n";
        text += "```\n";
        text += "Объект: пицца с пепперони на деревянном столе;\n";
        text += "освещение: тёплый боковой свет, аппетитные блики;\n";
        text += "ракурс: вид сверху, немного под углом;\n";
        text += "элементы: тянущийся сыр, базилик, пармезан, соус в пиале рядом;\n";
        text += "стиль: фуд-фотография, сочные цвета, глубина резкости малая;\n";
        text += "настроение: аппетитно, уютно.\n";
        text += "```\n\n";

        text += "🐱 *Животные в стиле фэнтези:*\n";
        text += "```\n";
        text += "Животное: пушистый рыжий кот;\n";
        text += "локация: волшебный лес с грибами и светлячками;\n";
        text += "элементы: у кота маленькие феи-крылышки, на голове цветочная корона;\n";
        text += "освещение: магическое свечение от грибов, мягкий рассеянный свет;\n";
        text += "стиль: сказочная иллюстрация, детализированно, акварельные тона;\n";
        text += "настроение: волшебство, доброта.\n";
        text += "```\n\n";

        text += "🏛️ *Архитектура в стиле модерн:*\n";
        text += "```\n";
        text += "здание: небоскрёб с зелёным фасадом (вертикальное озеленение);\n";
        text += "локация: деловой центр города, утро;\n";
        text += "ракурс: снизу вверх, слегка искажённая перспектива;\n";
        text += "освещение: солнечные лучи пробиваются сквозь листву на фасаде;\n";
        text += "элементы: отражение облаков в стекле, люди внизу, облака;\n";
        text += "стиль: архитектурная фотография, урбанистика, эко-технологии.\n";
        text += "```\n\n";

        text += "🎨 *Абстракция для фона:*\n";
        text += "```\n";
        text += "Стиль: абстрактный экспрессионизм;\n";
        text += "цвета: синий, золотой, изумрудный, переливы;\n";
        text += "текстура: мазки кистью, брызги, мраморные разводы;\n";
        text += "элементы: геометрические формы, плавные линии, градиенты;\n";
        text += "настроение: динамика, роскошь, глубина;\n";
        text += "использование: для фона презентации или обоев.\n";
        text += "```\n\n";

        text += "💡 *Советы:*\n";
        text += "• Указывайте ЧТО сохранить (лицо, поза, одежда)\n";
        text += "• Описывайте ЛОКАЦИЮ и ОСВЕЩЕНИЕ\n";
        text += "• Добавляйте ДЕТАЛИ (одежда, предметы, настроение)\n";
        text += "• Указывайте СТИЛЬ (фотореализм, арт, кино и т.д.)\n\n";

        text += "*Просто скопируйте промпт и используйте в боте!* 🚀";
